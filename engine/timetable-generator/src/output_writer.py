...
def _upsert_timetable(
    ...
    valid_status_json = '{"statuses":["VALID"],"details":{},"computedAt":"' + now.isoformat() + 'Z"}'
    ...
    if row:
        ...
        cur.execute("""
            ...
            SET status_json = %s::jsonb,
                ...
                updated_at = %s
            WHERE id = %s
        """, (valid_status_json, now, adjacency_constraint_enabled, now, now, timetable_id))
    else:
        ...
        """, (
            ...
            adjacency_constraint_enabled, now, now, now,
        ))
