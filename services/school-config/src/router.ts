...
export async function route(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  ...
  // Health check
  if (method === 'GET' && path === '/api/config/health') {
  ...
  const assignMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/assign$/);
  if (method === 'POST' && assignMatch) {
    return controller.assignToDivisions(event, assignMatch[1]);
  }

  // Match /config/period-structures/:id
  const psIdMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)$/);

  if (method === 'POST' && path === '/api/config/period-structures') {
    return controller.createPeriodStructure(event);
  }

  if (method === 'GET' && path === '/api/config/period-structures') {
  ...
  const workingDaysPutMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/working-days$/);
  ...
  const workingDaysGetMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/working-days$/);
  ...
  const copyDaySlotsMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/days\/([^/]+)\/copy-from\/([^/]+)$/);
  ...
  const reorderSlotsMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/days\/([^/]+)\/slots\/reorder$/);
  ...
  const updateSlotMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/days\/([^/]+)\/slots\/([^/]+)$/);
  ...
  const deleteSlotMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/days\/([^/]+)\/slots\/([^/]+)$/);
  ...
  const addSlotMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/days\/([^/]+)\/slots$/);
  ...
  const generateSlotsMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/slots\/generate$/);
  ...
  const getSlotsMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/slots$/);
  if (method === 'GET' && getSlotsMatch) {
    return controller.getSlots(event, getSlotsMatch[1]);
  }

  // POST /config/period-structures/:id/reset
  const resetMatch = path.match(/^\/api\/config\/period-structures\/([^/]+)\/reset$/);
